			...
			const struct open_flags *op, unsigned flags)
{
	struct file *file;
	int error;
	//pr_info("******path_openat 0");

	file = alloc_empty_file(op->open_flag, current_cred());
	if (IS_ERR(file))
		return file;
	//pr_info("******path_openat 1");

	if (unlikely(file->f_flags & __O_TMPFILE)) {
	//pr_info("******path_openat 2");
		error = do_tmpfile(nd, flags, op, file);
	} else if (unlikely(file->f_flags & O_PATH)) {
	//pr_info("******path_openat 3");
		error = do_o_path(nd, flags, file);
	} else {
	//pr_info("******path_openat 4");
		const char *s = path_init(nd, flags);
	//pr_info("******path_openat 5");
		while (!(error = link_path_walk(s, nd)) &&
		       (s = open_last_lookups(nd, file, op)) != NULL)
			;
	//pr_info("******path_openat 6");
		if (!error)
			error = do_open(nd, file, op);
	//pr_info("******path_openat 7");
		terminate_walk(nd);
	}
	//pr_info("******path_openat 8");
	if (likely(!error)) {
	...
	//pr_info("******path_openat 9");
	fput(file);
	//pr_info("******path_openat 10");
	if (error == -EOPENSTALE) {
		if (flags & LOOKUP_RCU)
			error = -ECHILD;
		else
		...
		const struct open_flags *op)
{
	...
	//pr_info("******do_filp_open 0");
	set_nameidata(&nd, dfd, pathname);
	//pr_info("******do_filp_open 1");
	filp = path_openat(&nd, op, flags | LOOKUP_RCU);
	//pr_info("******do_filp_open 2");
	if (unlikely(filp == ERR_PTR(-ECHILD)))
		filp = path_openat(&nd, op, flags);
	//pr_info("******do_filp_open 3");
	if (unlikely(filp == ERR_PTR(-ESTALE)))
		filp = path_openat(&nd, op, flags | LOOKUP_REVAL);
	//pr_info("******do_filp_open 4");
	restore_nameidata();
	//pr_info("******do_filp_open 5");
	return filp;
}
