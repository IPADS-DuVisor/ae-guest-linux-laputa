							...
							struct file *filp)
{
	...
	//pr_info("tty_port_open 0");
	++port->count;
	//pr_info("tty_port_open 1");
	spin_unlock_irq(&port->lock);
	//pr_info("tty_port_open 2");
	 ...
	 * port mutex.
	 */
	//pr_info("tty_port_open 3");

	mutex_lock(&port->mutex);
	//pr_info("tty_port_open 4 0x%lx", port->ops->activate);
	...
	//pr_info("tty_port_open 5");
		tty_port_set_initialized(port, 1);
	//pr_info("tty_port_open 6");
	}
	//pr_info("tty_port_open 7");
	mutex_unlock(&port->mutex);
	//pr_info("tty_port_open 8");
