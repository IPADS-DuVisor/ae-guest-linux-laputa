...
static int uart_port_startup(struct tty_struct *tty, struct uart_state *state,
		int init_hw)
{
	//pr_info("uart_port_startup 0");
	struct uart_port *uport = uart_port_check(state);
	unsigned long page;
	unsigned long flags = 0;
	int retval = 0;

	//pr_info("uart_port_startup 1");

	if (uport->type == PORT_UNKNOWN)
		return 1;
	//pr_info("uart_port_startup 2");

	/*
	 * Make sure the device is in D0 state.
	 */
	uart_change_pm(state, UART_PM_STATE_ON);
	//pr_info("uart_port_startup 3");
	...
	//pr_info("uart_port_startup 5 0x%lx", uport->ops->startup);

	retval = uport->ops->startup(uport); // serial8250_startup
	//pr_info("uart_port_startup 6");
	if (retval == 0) {
		...
		int init_hw)
{
	...
	//pr_info("uart_startup 0");
	if (tty_port_initialized(port))
		return 0;
	//pr_info("uart_startup 1");

	retval = uart_port_startup(tty, state, init_hw);
	if (retval)
		set_bit(TTY_IO_ERROR, &tty->flags);
	//pr_info("uart_startup 2");
...
static int uart_port_activate(struct tty_port *port, struct tty_struct *tty)
{
	...
	struct uart_port *uport;
	int ret;
	//pr_info("uart_port_activate 0");
	uport = uart_port_check(state);
	//pr_info("uart_port_activate 1");
	if (!uport || uport->flags & UPF_DEAD)
		return -ENXIO;
	//pr_info("uart_port_activate 2");

	/*
	 * Start up the serial port.
	 */
	//pr_info("uart_port_activate 3");
	ret = uart_startup(tty, state, 0);
	//pr_info("uart_port_activate 4");
	if (ret > 0)
		tty_port_set_active(port, 1);
	//pr_info("uart_port_activate 5");
